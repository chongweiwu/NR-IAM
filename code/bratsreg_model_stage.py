...
def CAM_negtive(feature, grads):
    b, c, _, _, _ = feature.shape
    grads = torch.mean(grads.view(b, c, -1), dim=2).view(b, c, 1, 1, 1)

    cam  = torch.sum(feature * grads, dim=1, keepdim=True)
    cam  = F.relu(-cam)
    cam_norm = cam / cam.max()
    return cam, cam_norm

def Grad_IAM(feature, grads):
    b, c, _, _, _ = feature.shape
    grads = torch.mean(grads.view(b, c, -1), dim=2).view(b, c, 1, 1, 1)

    cam  = feature * grads
    cam  = torch.sum(F.relu(-cam), dim=1, keepdim=True)
    cam_norm = cam / cam.max()
    return cam, cam_norm
...
class Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl1(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144), range_flow=0.4, num_block=5, diff=False):
    ...
    def forward(self, x, y, reg_code):
        ...
        if self.diff == True:
            output_disp_e0_v = self.diff_transform(output_disp_e0_v, self.grid_1)
...
class Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl2(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144), range_flow=0.4, model_lvl1=None, num_block=5, diff=False):
...
class Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl3(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144), range_flow=0.4, model_lvl2=None, num_block=5, diff=False):
...
class Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl3_cam(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144), range_flow=0.4, model_lvl2=None, num_block=5, diff=False):
        super(Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl3_cam, self).__init__()
        self.in_channel = in_channel
        self.n_classes = n_classes
        self.start_channel = start_channel
        ...
        self.conv_lvl1   = nn.Conv3d(self.start_channel * 8, self.start_channel * 4, kernel_size=3, stride=1, padding=1, bias=False)
        self.output_lvl1 = self.outputs(self.start_channel * 8, self.n_classes, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x, y, reg_code):
        ...
        fea1 = self.conv_lvl1(torch.cat([e0, fea_e0], dim=1))
        output_disp_e0_v = self.output_lvl1(fea1) * self.range_flow
        # output_disp_e0 = self.diff_transform(output_disp_e0_v, self.grid_1)
        if self.diff == True:
            output_disp_e0_v = self.diff_transform(output_disp_e0_v, self.grid_1)
        compose_field_e0_lvl1 = output_disp_e0_v + lvl2_disp_up
        ...
        if self.is_train is True:
            return compose_field_e0_lvl1, warpped_inputx_lvl1_out, y, output_disp_e0_v, lvl1_v, lvl2_disp, e0, fea1
            # return compose_field_e0_lvl1, warpped_inputx_lvl1_out, y, output_disp_e0_v, lvl1_v, lvl2_v, e0, lvl1_warp, lvl1_y, lvl2_warp, lvl2_y
        else:
            return compose_field_e0_lvl1
...
def create_windows(window_size, channel=3, batch_size=1):
        window_list = []
        Distance = torch.zeros(window_size**3, 3)

        m = window_size//2
        for i in range(window_size):
            for j in range(window_size):
                for k in range(window_size):
                    window = torch.zeros(batch_size, 1, *(window_size,)*channel)
                    window[:, :, i, j, k] = 1
                    window_list.append(window.cuda())

                    Distance[len(window_list)-1, 0] = i-m
                    Distance[len(window_list)-1, 1] = j-m
                    Distance[len(window_list)-1, 2] = k-m
        return window_list, Distance.abs().cuda()



def smoothloss(y_pred, weight=None):
    ...
    if weight==None:
        ...
        dy = torch.abs(y_pred[:,:,1:, :, :] - y_pred[:,:, :-1, :, :]) * weight[:,:,1:, :, :]
        dx = torch.abs(y_pred[:,:,:, 1:, :] - y_pred[:,:, :, :-1, :]) * weight[:,:,:, 1:, :]
        dz = torch.abs(y_pred[:,:,:, :, 1:] - y_pred[:,:, :, :, :-1]) * weight[:,:,:, :, 1:]
        out = (torch.sum(dx*dx)+torch.sum(dy*dy)+torch.sum(dz*dz))/(3.0 * torch.sum(weight))
        # dy = torch.abs(y_pred[:,:,1:, :, :] - y_pred[:,:, :-1, :, :]) 
        # dx = torch.abs(y_pred[:,:,:, 1:, :] - y_pred[:,:, :, :-1, :])
        # dz = torch.abs(y_pred[:,:,:, :, 1:] - y_pred[:,:, :, :, :-1])
        # out = (torch.mean(dy*dy * torch.exp(torch.abs(weight[:,:,1:, :, :] - weight[:,:, :-1, :, :])))+\
        #        torch.mean(dx*dx * torch.exp(torch.abs(weight[:,:,:, 1:, :] - weight[:,:, :, :-1, :])))+\
        #        torch.mean(dz*dz * torch.exp(torch.abs(weight[:,:,:, :, 1:] - weight[:,:, :, :, :-1]))))/3.0
    return out

def gradient_smoothloss(y_pred, fixed_img, weight=None, Wg=0.2):
    dy = torch.abs(y_pred[:,:,1:, :, :] - y_pred[:,:, :-1, :, :])
    dx = torch.abs(y_pred[:,:,:, 1:, :] - y_pred[:,:, :, :-1, :])
    dz = torch.abs(y_pred[:,:,:, :, 1:] - y_pred[:,:, :, :, :-1])

    gy = torch.abs(fixed_img[:,:,1:, :, :] - fixed_img[:,:, :-1, :, :])
    gx = torch.abs(fixed_img[:,:,:, 1:, :] - fixed_img[:,:, :, :-1, :])
    gz = torch.abs(fixed_img[:,:,:, :, 1:] - fixed_img[:,:, :, :, :-1])
    if weight==None:
        out = (torch.mean(dx*dx * torch.exp(- (Wg * gx)))+torch.mean(dy*dy * torch.exp(- (Wg * gy)))+torch.mean(dz*dz * -torch.exp(- (Wg * gz))))/3.0
    else:
        gy = gy * weight[:,:,1:, :, :]
        gx = gx * weight[:,:,:, 1:, :]
        gz = gz * weight[:,:,:, :, 1:]
...
def neg_Jdet_loss(y_pred, sample_grid, weight=None):
    neg_Jdet = -1.0 * JacboianDet(y_pred, sample_grid)
    selected_neg_Jdet = F.relu(neg_Jdet)
    
    if weight==None:
        out = torch.mean(selected_neg_Jdet)
    else:
        out = (selected_neg_Jdet * weight[:, 0, :-1, :-1, :-1]).sum() / weight.sum()
    return out
...
def dice_loss(input, target, i=1., r=1e-5):
        
        dice = torch.sum(target[input == i] == i) * 2.0 / (torch.sum(target == i) + torch.sum(input == i) + r)
        return - dice
...
class NCC_weight(torch.nn.Module):
    ...
    def forward(self, I, J, weight_map):
        ...
        # return negative cc.
        # return -1.0 * torch.sum(cc*weight_map) / torch.sum(weight_map)
...
class multi_resolution_NCC_allmod(torch.nn.Module):
    ...
    def  __init__(self, win=None, eps=1e-5, scale=3, channel=3):
        ...
        for i in range(scale):
            self.similarity_metric.append(NCC_allmod(win=win - (i*2), channel=channel))
            # self.similarity_metric.append(Normalized_Gradient_Field(eps=0.01))
...
class Dice_OASIS(torch.nn.Module):
    def __init__(self, atlas):
        super(Dice_OASIS, self).__init__()
        self.atlas = atlas
        self.unique_class = torch.unique(atlas)
    ...
    def dice(self, im1):
        ...
        for i in self.unique_class:
            if i == 0:
                continue
            sub_dice = torch.sum(self.atlas[im1 == i] == i) * 2.0 / (torch.sum(im1 == i) + torch.sum(self.atlas == i))
            dice += sub_dice
            num_count += 1
        return dice/num_count

    def forward(self, y_pred):
        return self.dice(y_pred)


class Dice_loss_OASIS(torch.nn.Module):
    def __init__(self, atlas):
        super(Dice_loss_OASIS, self).__init__()
        self.atlas = atlas
        self.unique_class = torch.unique(atlas)
